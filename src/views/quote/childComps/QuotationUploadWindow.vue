...
export default {
      ...
      template: [
        {
          columns: [
            ...
            },
            {
              ...
              columnWidth: '20%',
              init: function (component) {
                const button = jqwidgets.createInstance(
                  component,
                  'jqxButton',
                  {
                    width: 60,
  ...
  mounted() {
    ...
    this.$refs.reserveUploader.$on('changed', (data) => {
      delete this.detailFormData['projectName']
      delete this.detailFormData['projectAddress']
      delete this.detailFormData['customerCompany']
      delete this.detailFormData['customer']
      delete this.detailFormData['customerPhone']
      delete this.detailFormData['salesman']
      delete this.detailFormData['quoteDate']
      delete this.detailFormData['isRepeat']
      delete this.detailFormData['reservePrice']
      delete this.detailFormData['controlBoxReservePrice']
      ...
      for (const row of reserveFileContent) {
        for (const key in row) {
          ...
          // 业务员
          let salesman = cellData.match(/(?<=(报价人：|业务员：)).*?(?=(联系))/)
          if (salesman) {
            salesman = salesman[0]
    ...
    this.$refs.quoteUploader.$on('changed', (data) => {
      delete this.detailFormData['quotePrice']
      delete this.detailFormData['controlBoxQuotePrice']
      const sheetName = Object.keys(data[0])[0]
