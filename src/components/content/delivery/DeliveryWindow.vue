...
export default {
  ...
  mounted() {
    ...
    this.orderNumberInstance.jqxInput('disabled', this.orderNumberDisabled)
    // 项目名称
    this.projectNameInstance = this.$refs.myForm.getComponentByName(
      'projectName'
    )
    this.projectNameInstance.jqxInput('disabled', this.projectNameDisabled)
    // 送货日期
    ...
    $deliveryAmount
      ...
      .on('change', function () {
        ...
        const dlvWarranty = calc_misc_warranty(
          deliveryAmount,
          installFee,
          warranty
        )
  ...
  methods: {
    ...
    onValidationSuccess(event) {
      ...
      formData['deliveryReservePrice'] = this.deliveryReservePriceInstance.val()
